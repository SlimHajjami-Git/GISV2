...
public class ConfirmAccidentCommandHandler : IRequestHandler<ConfirmAccidentCommand, Unit>
{
    ...
    public async Task<Unit> Handle(ConfirmAccidentCommand request, CancellationToken ct)
    {
        ...
        // Calypso 7 — confirmation directly flips status to 'confirmed'.
        // The unified timeline (Phase 2 onwards) handles the rest of the
        // lifecycle (initial damages → expert → mechanic → repair →
        // insurance) on the /rapport-accident page. No intermediate
        // 'awaiting_details' state — each phase is its own optional step
        // editable independently and over multiple days.
        ev.Status = "confirmed";
