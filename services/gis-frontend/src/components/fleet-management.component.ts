              ...
              <div class="settings-group">
                ...
                <div class="remote-grid" *ngIf="!loadingRemote">
                  <div class="remote-card" *ngFor="let v of remoteVehicles" [class.online]="v.isOnline" [class.offline]="!v.isOnline">
                    ...
                    <div class="remote-immob-status" *ngIf="v.immobilizationActive">
                      <span class="immob-badge active">Immobilisé</span>
                      <span class="immob-by" *ngIf="v.immobilizationByName">par {{ v.immobilizationByName }}</span>
                    </div>

                    <div class="remote-card-actions" *ngIf="v.isOnline">
                      <button class="remote-btn stop" (click)="sendRemoteCmd(v, 'stop')" [disabled]="v.commandLoading || v.immobilizationActive" title="Arrêter le moteur">
                        ...
                        {{ v.commandLoading && v.commandType === 'stop' ? 'Envoi...' : 'Arrêter' }}
                      </button>
                      <button class="remote-btn resume" (click)="sendRemoteCmd(v, 'go')" [disabled]="v.commandLoading || !v.immobilizationActive" title="Redémarrer le moteur">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                        {{ v.commandLoading && v.commandType === 'go' ? 'Envoi...' : 'Démarrer' }}
                      </button>
                    </div>
                    <div class="remote-card-offline" *ngIf="!v.isOnline">
                      <span>Véhicule hors ligne</span>
                    </div>
...
export class FleetManagementComponent implements OnInit, OnDestroy {
      ...
      next: (data: any[]) => {
        this.remoteVehicles = (data || [])
          ...
          .map((v: any) => ({
            ...
            commandType: null as string | null,
            immobilizationActive: false,
            immobilizationByName: null as string | null
          }));
        ...
        // Load immobilization state for each vehicle with a device
        for (const v of this.remoteVehicles) {
          if (v.deviceId) {
            this.apiService.getImmobilizationState(v.deviceId).subscribe({
              next: (state: any) => {
                v.immobilizationActive = state.immobilizationActive;
                v.immobilizationByName = state.immobilizationByName;
                this.cdr.detectChanges();
              },
              error: () => {}
            });
  ...
  sendRemoteCmd(vehicle: any, command: 'stop' | 'go') {
    if (vehicle.commandLoading) return;

    const actionLabel = command === 'stop' ? 'ARRÊTER' : 'DÉMARRER';
    if (!confirm(`Voulez-vous vraiment ${actionLabel} le véhicule "${vehicle.name}" ?`)) return;

    vehicle.commandLoading = true;
    vehicle.commandType = command;
    vehicle.commandMessage = '';
    this.cdr.detectChanges();

    const obs = command === 'stop'
      ? this.apiService.stopVehicle(vehicle.deviceId)
      : this.apiService.goVehicle(vehicle.deviceId);

    obs.pipe(takeUntil(this.destroy$)).subscribe({
      next: (response: any) => {
        vehicle.commandLoading = false;
        vehicle.commandSuccess = true;
        vehicle.commandMessage = response.message || (command === 'stop' ? 'Commande STOP envoyée' : 'Commande GO envoyée');
        vehicle.immobilizationActive = command === 'stop';
        this.cdr.detectChanges();
