...
export class ExpensesComponent implements OnInit, OnDestroy {
  ...
  // Formulaire « Nouvelle dépense » : véhicule, catégorie, date, description,
  // montant. Depuis le 23/09/2026 (Karim), les natures qui ont leur propre écran
  // — carburant, entretien, réparation, assurance, visite technique, vignette —
  // ne se saisissent plus ici : les sous-formulaires dédiés ont été retirés.
  formVehicleId = '';
  ...
  private loadAllData(): void {
    this.loading = true;
    // Seuls les véhicules servent au formulaire : les types de carburant, les
    // modèles d'entretien, les fournisseurs et le catalogue de pièces n'ont plus
    // de sous-formulaire ici — cinq appels de moins à l'ouverture de l'écran.
  ...
  isFormValid(): boolean {
    return !!this.formVehicleId && !!this.selectedCategory && !!this.formDate && this.formAmount > 0;
  }
  ...
  /**
   * Un refus du serveur doit se VOIR. L'enregistrement se contentait d'un
   * console.error : le formulaire restait ouvert, rien n'apparaissait dans la
   * liste et l'utilisateur croyait à un bug d'affichage (recette du 08/09/2026,
   * constatée sur un plein refusé par la garde « un compteur ne recule pas » —
   * les pleins se saisissent depuis dans l'écran Carburant).
   */
