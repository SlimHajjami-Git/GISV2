...
public class AdminController : ControllerBase
{
    ...
    // ==================== DEVICE COMMANDS (ADMIN DIRECT) ====================

    [HttpPost("devices/bulk-command")]
    public async Task<IActionResult> SendBulkCommand([FromBody] BulkCommandRequest request)
    {
        if (request.DeviceIds == null || request.DeviceIds.Count == 0)
            return BadRequest(new { message = "Aucun device sélectionné" });

        if (string.IsNullOrWhiteSpace(request.CommandText))
            return BadRequest(new { message = "Commande vide" });

        // SAFETY: never allow STOP commands
        if (request.CommandText.Contains("STOP", StringComparison.OrdinalIgnoreCase))
            return BadRequest(new { message = "Les commandes STOP sont interdites depuis cette interface" });
        ...
        if (devices.Count == 0)
            return NotFound(new { message = "Aucun device trouvé" });
        ...
        foreach (var device in devices)
        {
            ...
            created.Add(new {
                ...
                vehiclePlate = device.Vehicle?.Plate,
        ...
        return Ok(new { message = $"{created.Count} commande(s) envoyée(s)", commands = created });
    }

    [HttpGet("devices/command-dashboard")]
    public async Task<IActionResult> GetCommandDashboard([FromQuery] int? companyId = null)
    {
        var vehicleQuery = _context.Vehicles
            .AsNoTracking()
            .Include(v => v.GpsDevice)
        ...
        // Batch: latest command per device
        var latestCommands = await _context.DeviceCommands
            .AsNoTracking()
            .Where(c => deviceIds.Contains(c.DeviceId))
            .GroupBy(c => c.DeviceId)
            .Select(g => g.OrderByDescending(c => c.CreatedAt).First())
            .ToDictionaryAsync(c => c.DeviceId);
        ...
        var result = vehicles.Select(v => {
            ...
            latestCommands.TryGetValue(device.Id, out var lastCmd);
            ...
            return new {
                ...
                lastCommand = lastCmd != null ? new {
                    id = lastCmd.Id,
                    commandType = lastCmd.CommandType,
                    commandText = lastCmd.CommandText,
                    status = lastCmd.Status,
                    sentAt = lastCmd.SentAt,
                    createdAt = lastCmd.CreatedAt,
                    source = lastCmd.Source,
                    errorMessage = lastCmd.ErrorMessage
                } : null
            };
    ...
    [HttpGet("devices/command-history")]
    public async Task<IActionResult> GetCommandHistory([FromQuery] int limit = 100, [FromQuery] int? companyId = null)
    {
        var query = _context.DeviceCommands
            .AsNoTracking()
            .AsQueryable();
