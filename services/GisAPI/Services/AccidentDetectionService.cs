...
";
        ...
        CancellationToken ct)
    {
        ...
        var ev = new AccidentEvent
        {
            ...
            ReasonsJson = JsonSerializer.Serialize(BuildReasons(candidate), jsonOpts),
            IndicatorsJson = JsonSerializer.Serialize(BuildIndicators(candidate), jsonOpts),
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Narrative builders — the frontend /rapport-accident/{id} page
    // expects Story/Reasons/Indicators as structured JSON. We generate a
    // default narrative from the raw signal so the report is already
    // meaningful on first render, before any reverse geocoding or human
    // review.
    // ─────────────────────────────────────────────────────────────────────

    private static object[] BuildStory(AccidentCandidate c)
    {
        ...
        return new object[]
        {
            new {
                ...
                body  = $"Le véhicule circule à environ {Math.Round(c.KphBef)} km/h dans les minutes précédant l'incident. Aucun comportement anormal n'est détecté.",
                severity = "normal"
            },
            new {
            ...
            new {
                ...
                body  = $"Les capteurs restent au-delà de leur seuil haut pendant plusieurs secondes (magnitude globale {Math.Round(c.Mag)} sur une échelle où 128 est la saturation d'un axe).",
                severity = "critical"
            },
            new {
                time  = impact.AddMinutes(5).ToString("HH'h' mm"),
                title = "Immobilisation complète",
                body  = $"Dans les minutes qui suivent, le véhicule est strictement à l'arrêt (vitesse maximale observée : {Math.Round(c.KphAft)} km/h sur {c.NAft} relevés).",
                severity = "warning"
            }
        };
    }

    private static object[] BuildReasons(AccidentCandidate c) => new object[]
    {
        new {
            title = "Chute brutale et incontrôlée de la vitesse",
            text  = $"Le véhicule passait de {Math.Round(c.KphBef)} km/h à une immobilisation totale en quelques secondes. Ce profil ne correspond pas à un freinage volontaire."
        },
        new {
    ...
    private static object[] BuildIndicators(AccidentCandidate c) => new object[]
    {
        new {
        ...
        new {
            ...
            value = Math.Round(c.Mag).ToString(),
    ...
    private static string BuildSynthesisText(AccidentCandidate c)
    {
